function [] = runSO_injured(file_path,genericSetupForSO,modelFilePath,cutoff,in_leg)
...
for m = 1:length(trials)
    ...
    name = regexprep(IKFile,'_IK_injured.mot','_injured');
...
% Get the trials of injured leg walking
if in_leg == 1 %if injured is is right, injured is left
    trials = dir(fullfile(file_path,'\IK\lungeH*injured.mot'));
else
    trials = dir(fullfile(file_path,'\IK\lungeV*injured.mot'));
end
