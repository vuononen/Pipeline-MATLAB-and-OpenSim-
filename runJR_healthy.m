function [] = runJR_healthy(file_path,genericSetupForJR,modelFilePath,cutoff,in_leg)
...
trials = dir(fullfile(file_path,'\SO\gang*healthy_Static*force.sto'));
...
for m = 1:length(trials)
    ...
    analyzeTool.setCoordinatesFileName(fullfile([SO_data_folder(1:end-2) 'IK\'],[SOFile(1:end-36) 'IK_healthy.mot']));
...
trials = dir(fullfile(file_path,'\SO\lunge*healthy_Static*force.sto'));
